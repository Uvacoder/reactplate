...
declare const global: TestSetupGlobal;
...
beforeAll(async () => {
  ...
  try {
    ...
    const testName = slash(testPath).match(/examples\/([\w-]+)\//)?.[1];

    if (testName) {
      const rootPath = resolve(__dirname, "../");
      const srcDir = resolve(rootPath, testName);
      ...
      if (fs.existsSync(testCustomServe)) {
        ...
        server = await serve(path.join(rootDir, "../../"), isBuildTest);
        return;
      }
